...
/**
 * This class can be used to merge pklfiles with the original PKL
 * filename in the header and can store the original files in the DB
 * when desired.
 *
...
public class PKLMergerAndStorer {
    ...
    /**
     * Key for the total number of PKL files processed.
     * This key is present in the HashMap with the stats,
     * returned by 'mergeAllFilesFromFolderToFolder.
     */
    public static final String TOTAL_NUMBER_OF_FILES = "TOTAL_NUMBER_OF_FILES";
    /**
     * Key for the total number of merge files produced.
     * This key is present in the HashMap with the stats,
     * returned by 'mergeAllFilesFromFolderToFolder.
     */
    ...
    /**
     * This boolean indicates whether the individual PKL files should be
     * stored in the DB.
     */
    ...
    /**
     * This constructor allows the storage of the individual PKL files
     * in the DB before merging.
     *
     * @param   aStoreInDB  boolean to indicate whether or not to store the
     *                      individual PKL files in the DB.
     * @param   aDBParams   HashMap with the connection parameters for the DB
     *                      but 'null' can be allowed when aStoreInDB is 'false'.
     *                      Required parameters are DRIVER and URL, and all other
     *                      parameters present in the HashMap will be forwarded to the
     *                      DB at connection time in the same key-value format they have
     *                      in the HashMap.
     */
    ...
    /**
     * This method merges the specified PKL files into a String.
     * It adds a few blanks (three) between each spectrum and
     * adds the original filename to the first line (header), separated
     * by a space.
     *
     * @param   aInputFiles File[] with the PKL files to merge.
     * @return  String  with the merged content
     * @throws  java.io.IOException   whenever the input files could not be read.
     ...
     * @throws java.lang.Exception  whenever the output files could not be written (IOException),
     *                    database problems occur (SQLException) or the DB driver cannot be found (various).
     */
    public static HashMap mergeFilesFromDBConnectionToFile(File aDestinationFolder, int aSize, Properties aParams, String aWhereClause) throws Exception {
        ...
        Connection conn = d.connect(aParams.getProperty(PKLMergerAndStorer.URL), aParams);
        PreparedStatement ps = conn.prepareStatement(query.toString());
        ResultSet rs = ps.executeQuery();
        Vector tempVec = new Vector(aSize);
        while(rs.next()) {
            Spectrumfile pkl = new Spectrumfile(rs);
            StringBuffer tempSB = new StringBuffer(new String(pkl.getUnzippedFile()));
    ...
    /**
     * This method merges the specified PKL files into a new physical file.
     * It adds a few blanks (three) between each spectrum and
     * adds the original filename to the first line (header), separated
     * by a space.
     *
     ...
     * @throws  java.io.IOException   whenever the input files could not be read, or the
    ...
    /**
     * This method allows the caller to specify an input folder and an output folder
     * as well as a size. The program will then take all files currently in the folder and
     * merge them.
     *
     ...
     * @return  HashMap with some output stats for the curious caller.
     * @exception   java.io.IOException when the merging was not possible.
     */
    ...
    /**
     * This method allows the caller to specify an input folder and an output folder
     * as well as a size. The program will then take all files currently in the folder
     * (taking into account a possible file filter) and merge them.
     *
     * @param   aSourceFolder   File with the location of the source folder (must point to a directory!)
     * @param   aDestinationFolder   File with the location of the destination folder (must point to a directory!)
     * @param   aSize   int with the maximum number of PKL files per merge file.
    ...
    /**
     * This method merges the Strings in the specified Vector to a mergefile in the specified
     * output folder, where the filename is: merge_[suffix].txt. <br />
     * Note that ALL the Strings in the Vector are merged, regardless of their number.
     *
    ...
    /**
     * This method prints the specified message to stderr and then
     * exits the JVM, setting the status flag to '1'.
     *
    ...
    /**
     * This method will find all files in the specified folder, corresponding to
     * the specified filename filter (if any) and the search can be recursive.
     *
     * @param aSourceFolder File with the sourcefolder.
     * @param aFileFilter   String with the filename filter to apply (can be 'null' for
     *                      no filter).
    ...
    /**
     * This method will allow recursive 'mining' of a folder for (filtered)
     * files.
     *
