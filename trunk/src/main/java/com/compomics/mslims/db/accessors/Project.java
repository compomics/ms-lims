 ...
 *   <li><b>clone()</b>: returns an identical copy of the project.</li>
 * </ul>
 *
...
public class Project extends ProjectTableAccessor {
     ...
     * Column 2: link to the person responsible for the project <br />
     * Column 3: link to the PROTOCOL type used for the project <br />
     * Column 4: title of the project <br />
     * Column 5: the description for the project
    ...
    public static Project[] getAllDifferentialProjects(Connection aConn) throws SQLException {
        PreparedStatement prep = aConn.prepareStatement("select distinct p.projectid, p.l_userid, p.l_protocolid, p.title, p.description, p.username, p.creationdate, p.modificationdate from project as p, spectrumfile as s, identification as i where p.projectid=s.l_projectid and s.spectrumfileid=i.l_spectrumfileid and i.light_isotope>0 and i.heavy_isotope>0 order by p.creationdate desc, p.title asc");
        ResultSet rs = prep.executeQuery();
        Vector v = new Vector();
        while(rs.next()) {
            v.add(new Project(rs));
