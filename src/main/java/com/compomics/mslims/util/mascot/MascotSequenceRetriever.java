...
/**
 * This class will contact a Mascot server (the one in the properties file) and request the FASTA sequence for the
 * specified accession number.
 *
 * @author Lennart Martens
 */
public class MascotSequenceRetriever {
    // Class specific log4j logger for MascotSequenceRetriever instances.
    ...
    /**
     * This constructor takes the filename for the propertiesfile as argument and reads the server URL from the 'URL'
     * key. The properties file should be in the classpath and it parses the value behind the 'URL' key such that it
     * should start with 'HTTP://', then the server name, then '/' and then 'mascot'. <br /> If any of this fails, a
     * RuntimeException is thrown.
     *
     * @param aPropertiesFile String  with the propertiesfile to read.
     * @throws IllegalArgumentException when the propertiesfile was incorrectly formatted, or the value in the
     *                                  propertiesfile was incorrectly formatted (see above).
     */
    ...
    /**
     * This constructor allows the creation of a MascotSequenceRetriever through the specification of a Mascot server
     * hostname and an optional database name.
     *
    ...
    /**
     * This method reads the results from the server for the specified accession number. It also automatically
     * determines the database that needs be searched.
     *
     * @param aAccession String with the accession number to look up.
     * @return String with the FASTA formatted sequence.
     * @throws IOException when the communication with the server broke down, or no hit was found for the specified
     *                     accession number.
     */
    ...
    /**
     * This method returns a reader for the results from the server for the specified accession number in the specified
     * mode. <br /> It also automatically determines the database that needs be searched.
     *
     * @param aAccession String with the accession number to look up.
    ...
    /**
     * This method can be used to retrieve a set of FASTA sequences from a set of accessions.<br /> It results in a
     ...
     * of calls to the 'getRawSequence' method, so no network optimization is done.
     *
    ...
    public String[] getSequences(Vector aAccessions) {
        ...
        for (int i = 0; i < liSize; i++) {
            try {
                ...
                logger.error("Failed to retrieve sequences: " + ioe.getMessage());
            }
        }
        return allResults;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < result.length; i++) {
            ...
            if (s != null) {
                ...
                logger.error(accessions.get(i) + ": result was 'null'!!");
            }
        }
    }
}
