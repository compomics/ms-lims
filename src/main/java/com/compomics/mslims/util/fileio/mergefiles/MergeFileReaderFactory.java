...
public class MergeFileReaderFactory {
    ...
    /**
     * The constant holding the filename for the properties file from the available MergeFileReaders
     * will be read.
     */
    private static final String PROPERTIES_FILE = "mergefilereaders.properties";

    /**
     * The constant String used to separate the properties key from the classname in the HashMap
     * key.
     */
    private static final String SEPARATOR = "$$-*-$$";

    /**
     * This HashMap will contain all the available mergefilereaders.
     * Mappings it will contain are (key, value): ('properties_key + SEPARATOR + classname', testing_instance_of_class).
     */
    ...
    static {
        try {
            ...
            InputStream is = MergeFileReaderFactory.class.getClassLoader().getResourceAsStream(PROPERTIES_FILE);
            // See if it was succesful.
            if(is == null) {
                throw new IOException("File '" + PROPERTIES_FILE + "' was not found in the classpath!");
            } else {
                // Load the properties in the file.
     ...
     * @throws java.io.IOException  when the file was either not found, could not be opened or had an unrecognized format.
     */
