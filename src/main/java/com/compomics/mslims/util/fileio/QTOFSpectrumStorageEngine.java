 ...
 * to load and store CapLC LC runs and corresponding PKL files from a Micromass Q-TOF mass spectrometer.
 *
...
public class QTOFSpectrumStorageEngine implements SpectrumStorageEngine {
    ...
    public int loadAndStoreSpectrumFiles(LCRun aLCRun, long aProjectid, long aInstrumentid, Connection aConn) throws IOException, SQLException {
        ...
        for(int i = 0; i < pklFiles.size(); i++) {
            ...
            HashMap lhm = new HashMap(9);
            // The filename has to change extension from 'pkl' to 'mgf'.
            String tempFilename = filename.substring(0, filename.lastIndexOf(".")) + ".mgf";
            lhm.put(Spectrumfile.FILENAME, tempFilename);
            // The total intensity.
            lhm.put(Spectrumfile.TOTAL_SPECTRUM_INTENSITY, pklFile.getTotalIntensity());
            // The highest intensity.
            lhm.put(Spectrumfile.HIGHEST_PEAK_IN_SPECTRUM, pklFile.getHighestIntensity());
            lhm.put(Spectrumfile.IDENTIFIED, new Long(0));
            lhm.put(Spectrumfile.L_LCRUNID, new Long(lCaplcID));
            lhm.put(Spectrumfile.L_PROJECTID, new Long(aProjectid));
            lhm.put(Spectrumfile.L_INSTRUMENTID, new Long(aInstrumentid));
            lhm.put(Spectrumfile.SEARCHED, new Long(0));
            Spectrumfile pkl = new Spectrumfile(lhm);
            // Note that we transform the file contents to bytes using the
            // platforms default encoding.
            pkl.setUnzippedFile(pklFile.getMGFContents().getBytes());
            pkl.persist(aConn);
            counter++;
        }
