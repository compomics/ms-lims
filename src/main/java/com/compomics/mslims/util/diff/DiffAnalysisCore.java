...
/**
 * This class carries out a full, non-redundant (at the peptide sequence level)
 * differential analysis for any number of projects. The analysis is based on the
 * robust Huber scale estimator and the median for location.
 *
...
public class DiffAnalysisCore {
    /**
     * This variable causes the average ratio for merged couples to be calculated as the
     * weighted average ratios. The calculation comprises several stages.
     * First, all the individual ratios are calculated, then, all light intensities are summed and the
     * relative contribution of each light intensity is calculated for each couple. The same is done for the
     * heavy intensities. Next, all ratios are multiplied by this relative contribution for total light intensity
     * and the results are summed. the same applies for the heavy intensities. We now have two weighted average ratios:
     * one for heavy and one for light intensities. The final result is the average of these two.<br />
     * <i>Example:</i> <br />
     * (34,11), (23,12), (36, 20), (48, 15) averaged with WEIGHTED_RATIOS,
     * yields a ratio of: 2.503.
     * <br />
     * A very intense yet aberrant signal still influences the average. <br />
     * <i>Example:</i> <br />
     * (1234,1311), (23,12), (36, 20), (48, 15) averaged with WEIGHTED_RATIOS,
     * yields the a ratio of: 1.034.
     * The reported average is thus weighted.
     */
    public static final int WEIGHTED_RATIOS = 0;

    /**
     * This variable causes the average ratio for merged couples to be calculated as the
     * average of all the individual ratios.<br />
     * <i>Example:</i> <br />
     * (34,11), (23,12), (36, 20), (48, 15) averaged with AVERAGE_RATIOS,
     * yields the individual ratios (3.09), (1.92), (1.80), (3.20). The average
     * ratio then is: 2.5025.
     * <br />
     * A very intense yet aberrant signal no longer greatly influences the average. <br />
     * <i>Example:</i> <br />
     * (1234,1311), (23,12), (36, 20), (48, 15) averaged with AVERAGE_RATIOS,
     * yields the individual ratios of (0.94), (1.92), (1.80), (3.20). The average
     * ratio then is: 1.965.
     * You can see that the first measurement (ratio appr. 1) influences the trend in
     * the subsequent measurements only as one out of four measurements (ratios appr. 2.5).
     * The reported average is not weighted.
     */
    ...
    /**
     * The type of statistics used. Can be evaluated against the
     * X_STATISTICS constants defined on this class.
     */
    private int iStatType = -1;

    /**
     * The averaging method applied when calculating ratios for merged couples.
     * Should be equal to one of the constants defined on the DiffCouple class.
     */
    ...
    /**
     * This constructor takes the connection to read data from and
     * the calibrated standard deviation of the instrument as arguments.
     * The averaging method can be chosen.
     *
     * @param aConn Connection with the DB connection to read data from.
     * @param aCalibratedStDev  double with the calibrated standard deviation
     *                          of the instrument for a 1/1 sample.
     * @param aStatType int with the type of statistics that this instance will calculate.
     *                  Use the constants defined on this class (ROBUST_STATISTICS or STANDARD_STATISTICS).
     * @param aAveragingMethod  int with the averaging method for merged couples. Use the constants defined on this
     *                          class (SUM_INTENSITIES or AVERAGE_RATIOS).
     * @param aRecenter Double with the value to recenter each project to. Can be 'null' for no recentering.
     * @param aWhereAddition    String to append to the whereclause. Will be prefixed with ' AND ' before appending.
     *                          Can be 'null' for no addition to the default clause.
     */
     ...
     * This constructor takes the connection to read data from and
     * the calibrated standard deviation of the instrument as arguments.
     * The averaging method can be chosen and a recentering point can be specified.
     *
     * @param aConn Connection with the DB connection to read data from.
     * @param aCalibratedStDev  double with the calibrated standard deviation
     *                          of the instrument for a 1/1 sample.
     ...
     * This constructor takes the connection to read data from and
     * the calibrated standard deviation of the instrument as arguments.
     * The averaging method defaults to DiffCouple.SUM_INTENSITIES.
     *
     * @param aConn Connection with the DB connection to read data from.
     * @param aCalibratedStDev  double with the calibrated standard deviation
     *                          of the instrument for a 1/1 sample.
    ...
    /**
     * This method reads all the differential data from the specified instrument for the specified projects
     * and stores the results in the results HashMap, which is treated as a reference parameter.
     * The results in the HashMap are keyed by the constants defined on this class
     * (MU_HAT, SIGMA_HAT, INSTRUMENT_STDEV, COUNT, ITERATIONS, DIFFCOUPLES, AVERAGING_METHOD).
     *
    ...
    /**
     * This method reads all identifications from the specified machine that have differential couple data for
     * each specified project from the DB.
     * The Identification instances are transformed into DiffCouple instances and subsequently combined in the aCombined
     * Vector (which is treated as a reference variable)!
     *
            ...
            if(ids != null && ids.length > 0) {
                ...
                for (int j = 0; j < ids.length; j++) {
                    ...
                    if(lProject.isInverse()) {
                        // Inverse
                        dc = new DiffCouple(lProject.getProjectID(), 1, lID.getHeavy_isotope().doubleValue(), lID.getLight_isotope().doubleValue(), ""+lID.getL_spectrumfileid(), lID.getAccession(), (int)lID.getStart(), (int)lID.getEnd(), lID.getEnzymatic(), lID.getSequence(), lID.getModified_sequence(), lID.getDescription());
                    } else {
                        // Normal.
                        dc = new DiffCouple(lProject.getProjectID(), 1, lID.getLight_isotope().doubleValue(), lID.getHeavy_isotope().doubleValue(), ""+lID.getL_spectrumfileid(), lID.getAccession(), (int)lID.getStart(), (int)lID.getEnd(), lID.getEnzymatic(), lID.getSequence(), lID.getModified_sequence(), lID.getDescription());
                    }
     ...
     * @return  Vector with non-redundant DiffCouples. All entries that have been merged,
     *                 retain references to each merged entry.
     */
