...
public class Datfiles_and_Identification_to_50DBConverterStepImpl implements DBConverterStep {
    ...
    public boolean performConversionStep(Connection aConn) {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                rs.close();
                if(counter == 0) {
                    System.err.println("\t # Unable to find an identification for datfile with filename '" + key + "' and datfileid " + datfileid + "! (" + datfilesDoneCount + " datfiles done already)");
                } else if(counter > 1) {
                    System.err.println("\t # Found more than one an server for datfile with filename '" + key + "' and datfileid " + datfileid + "! (" + datfilesDoneCount + " datfiles done already)");
                } else {
                    // Update the datfile.
                    ...
                    if(affected != 1) {
                        System.err.println("\t # Updated " + affected + " rows instead of one for original datfile filename '" + key + "'!");
                    }
            ...
            PreparedStatement psFindSpectrumfileid = aConn.prepareStatement("select spectrumfileid from spectrumfile where filename=?");
            ...
            for (Iterator lIterator = filenames.iterator(); lIterator.hasNext();) {
                ...
                if(specFile) {
                    ...
                    psUpdateIdentification.setLong(1, spectrumfileid);
                    psUpdateIdentification.setString(2, filename);
                    psUpdateIdentification.executeUpdate();
                    psUpdateIdentification.clearParameters();
                } else {
                    rs.close();
                    System.err.println("\t # Did not find a spectrumfileid for spectrum filename '" + filename + "'!");
                } 
            }
    ...
    private class InnerData {
        ...
        /**
         * This constructor takes the original data (filename and server) for the datfile and
         * parses these into a filename, folder and server.
         *
