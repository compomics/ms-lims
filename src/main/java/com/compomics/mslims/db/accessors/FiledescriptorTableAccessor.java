...
/**
 * This class is a generated accessor for the Filedescriptor table.
 *
...
public class FiledescriptorTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
	...
	/**
	 * This variable represents the contents for the 'filedescriptorid' column.
	 */
	protected long iFiledescriptorid = Long.MIN_VALUE;


	/**
	 * This variable represents the contents for the 'short_label' column.
	 */
	protected String iShort_label = null;
	...
	/**
	 * This variable represents the key for the 'filedescriptorid' column.
	 */
	public static final String FILEDESCRIPTORID = "FILEDESCRIPTORID";

	/**
	 * This variable represents the key for the 'short_label' column.
	 */
	...
	/**
	 * This constructor allows the creation of the 'FiledescriptorTableAccessor' object based on a set of values in the HashMap.
	 *
	...
	public FiledescriptorTableAccessor(HashMap aParams) {
		if(aParams.containsKey(FILEDESCRIPTORID)) {
			this.iFiledescriptorid = ((Long)aParams.get(FILEDESCRIPTORID)).longValue();
		}
		if(aParams.containsKey(SHORT_LABEL)) {
			this.iShort_label = (String)aParams.get(SHORT_LABEL);
		}
	...
	/**
	 * This method returns the value for the 'Filedescriptorid' column
	 * 
	 * @return	long	with the value for the Filedescriptorid column.
	 */
	...
	/**
	 * This method returns the value for the 'Short_label' column
	 * 
	 * @return	String	with the value for the Short_label column.
	 */
	...
	public int delete(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("DELETE FROM filedescriptor WHERE filedescriptorid = ?");
		lStat.setLong(1, iFiledescriptorid);
	...
	public void retrieve(Connection aConn, HashMap aKeys) throws SQLException {
		...
		if(!aKeys.containsKey(FILEDESCRIPTORID)) {
			throw new IllegalArgumentException("Primary key field 'FILEDESCRIPTORID' is missing in HashMap!");
		} else {
			iFiledescriptorid = ((Long)aKeys.get(FILEDESCRIPTORID)).longValue()
;		}
		// In getting here, we probably have all we need to continue. So let's...
		PreparedStatement lStat = aConn.prepareStatement("SELECT * FROM filedescriptor WHERE filedescriptorid = ?");
		lStat.setLong(1, iFiledescriptorid);
		ResultSet lRS = lStat.executeQuery();
		int hits = 0;
		while(lRS.next()) {
			hits++;
			iFiledescriptorid = lRS.getLong("filedescriptorid");
			iShort_label = (String)lRS.getObject("short_label");
	...
	public int update(Connection aConn) throws SQLException {
		...
		PreparedStatement lStat = aConn.prepareStatement("UPDATE filedescriptor SET filedescriptorid = ?, short_label = ?, description = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE filedescriptorid = ?");
		lStat.setLong(1, iFiledescriptorid);
		lStat.setObject(2, iShort_label);
		lStat.setObject(3, iDescription);
		lStat.setObject(4, iUsername);
		lStat.setObject(5, iCreationdate);
		lStat.setLong(6, iFiledescriptorid);
		int result = lStat.executeUpdate();
		lStat.close();
		this.iUpdated = false;
		return result;
	...
	public int persist(Connection aConn) throws SQLException {
		PreparedStatement lStat = aConn.prepareStatement("INSERT INTO filedescriptor (filedescriptorid, short_label, description, username, creationdate, modificationdate) values(?, ?, ?, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)");
		if(iFiledescriptorid == Long.MIN_VALUE) {
			lStat.setNull(1, 4);
		} else {
			lStat.setLong(1, iFiledescriptorid);
		}
		if(iShort_label == null) {
			lStat.setNull(2, 12);
		} else {
			lStat.setObject(2, iShort_label);
		}
