...
/**
 * This class provides a wrapper around a LCRunTableAccessor.
 * It defines a comparison (via a Comparable implementation), a toString
 * (for use in listviews), an 'assigned' variable (for use in listviews as well).
 *
...
public class LCRun extends LcrunTableAccessor implements Comparable {
    ...
    /**
     * This boolean indicates whether this LC run has been assigned to a
     * project already.
     */
    ...
    /**
     * This constructor takes the LC run name (eg., 'CapLCxxxx' with each 'x'
     * a digit for the Micromass Q-TOF) and the number of spectra (eg., PKL files
     * for the Micromass Q-TOF) in that LC run.
     *
     * @param aName String with the name for the LC run (typically 'CapLCxxxx' with each 'x'
     *              a digit for the Micromass Q-TOF).
    ...
    /**
     * This constructor reads the LC run from a resultset. The ResultSet should be positioned such that
     ...
     * Column 1: lcrunid <br />
     * Column 2: link to projectid <br />
     * Column 3: description <br />
     * Column 4: number of files at storage time <br />
     * Column 5: name for the LC run <br />
     * Column 6: cd number <br />
     * Column 7: dvd number <br />
     * Column 8: primary fraction number <br />
    ...
    /**
     * This method retrieves all unique LC run names from the database, whose creationdate is no more than
     * the specified number of days in the past.
     * .
     * @param aConn Connection to read the LC runs from.
     * @param aDaysAgo  int with the maximum age (creationdate may not be more then aDaysAgo days ago than today)
     *                  for the LC runs (-1 retrieves all).
    ...
    /**
     * This method reports an LCRun[] with all LCRuns that have not been assigned a primary fraction
     * yet, and that are younger than the specified maximum age in days (comparing CURRENT_TIMESTAMP
     * to creationdate).
     *
    ...
    /**
     * This method reports an LCRun[] with all LCRuns that are selected using the specified
     * whereclause.
     *
     * @param aConn Connection to read the LCRuns from.
