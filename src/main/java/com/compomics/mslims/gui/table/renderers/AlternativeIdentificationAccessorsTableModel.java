 ...
 */
public class AlternativeIdentificationAccessorsTableModel extends AbstractTableModel {
    private static Logger logger = Logger.getLogger(AlternativeIdentificationAccessorsTableModel.class);

    /**
     * The data to display.
     */
    private AlternativeIdentification[] iData = null;
     ...
     */
    public AlternativeIdentificationAccessorsTableModel(Vector aCandidiates) {
        // First cycle and count.
        int count = 0;
        int liSize = aCandidiates.size();
        for (int i = 0; i < liSize; i++) {
            Object o = aCandidiates.get(i);
            if (o instanceof AlternativeIdentificationTableAccessor) {
                count++;
            }
        }
        // Okay, init the array.
        iData = new AlternativeIdentification[count];
        count = 0;
        for (int i = 0; i < liSize; i++) {
            Object o = aCandidiates.get(i);
            if (o instanceof AlternativeIdentificationTableAccessor) {
                iData[count] = (AlternativeIdentification) o;
                count++;
            }
        }
    }
