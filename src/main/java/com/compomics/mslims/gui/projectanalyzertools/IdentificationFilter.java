...
/**
 * Created by IntelliJ IDEA. User: niklaas Date: 25-feb-2009 Time: 8:24:18 To change this template use File | Settings |
 * File Templates.
 */
public class IdentificationFilter extends JFrame implements ProjectAnalyzerTool {
    // Class specific log4j logger for IdentificationFilter instances.
    private static Logger logger = Logger.getLogger(IdentificationFilter.class);
    ...
    /**
     * Vector with the protein sequences. The sequence found in this Vector on a specific position is linked to the
     * protein accession in the iProteinAccession Vector on the same position.
     */
                ...
                try {
                    ...
                    for (int i = 0; i < iIdentifications.length; i++) {
                        ...
                        for (int j = 0; j < lPeptideAccessions.size(); j++) {
                            ...
                            if (lSequence == null) {
                                logger.error("Did not find protein sequence for: " + lAccession);
                            } else {
                                ...
                                if (lStart == -1 || lStart == 0) {
                                    logger.info(iIdentifications[i].getSequence() + " could not be found in " + lPeptideAccessions.get(j));
                                } else {
    ...
    /**
     * This method will check the amino acid input Rules: - The amino acids must be seperated by a comma - The amino
     * acids must be in the one letter form - If the amino acids are modified it must be like : "K<Ace>"
     *
