...
/**
 * This class implements a GUI to interface with a FolderMonitor that will FTP all new files
...
public class FTPSatellite extends JFrame implements Logger, PickUp {
    ...
    /**
     * The object that will allow us to merge and store
     * PKL files.
     */
    ...
    /**
     * This constructor takes a title for the JFrame.
     * It also constructs the screen.
     *
    ...
    /**
     * This method is called by the SettingsDialog to intialize
     * the FTP parameters.
     *
    ...
    /**
     * This method is called by the SettingsDialog to initialize
     * the DB parameters.
     *
    ...
    /**
     * This method sets all the monitoring parameters.
     * It is called by the SettingsDialog.
     *
     * @param   aFolder File which points to the directory that has to be monitored.
     * @param   aFilter String with the filename extensions to filter (can be 'null'
     *                  whenever no filter is desired).
     ...
     *
     * @param   aInitialized    boolean to indicate whether the settings have been entered
     *                          by the user and have been transferred.
     */
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                while(!sure) {
                    ...
                    if(operation == JFileChooser.APPROVE_OPTION) {
                        ...
                        if(f.exists()) {
                            if(f.isDirectory()) {
                                JOptionPane.showMessageDialog(FTPSatellite.this, "You selected a folder, not a file!", "Folder selected instead of file!", JOptionPane.ERROR_MESSAGE);
                                sure = false;
                            } else {
                            ...
                            try {
                                ...
                                f = null;
                                JOptionPane.showMessageDialog(FTPSatellite.this, "Unable to create the file you requested!", "Unable to create file!", JOptionPane.ERROR_MESSAGE);
                            }
                        }
                    }
                }
    ...
    private void startPressed() {
        ...
        if(this.iDriver == null) {
            ...
            if(iMerge) {
                ...
                if(iFilter == null) {
                    iFM = new FolderMonitor(iFolder, iInterval, FolderMonitor.GATHER_FILES_FOR_PICKUP, params, this);
                } else {
                    iFM = new FolderMonitor(iFolder, iInterval, iFilter, FolderMonitor.GATHER_FILES_FOR_PICKUP, params, this);
                }
            } else {
                ...
                if(iFilter == null) {
                    iFM = new FolderMonitor(iFolder, iInterval, FolderMonitor.FTP_TO_SPECIFIED_DESTINATION, params, this);
                } else {
                    iFM = new FolderMonitor(iFolder, iInterval, iFilter, FolderMonitor.FTP_TO_SPECIFIED_DESTINATION, params, this);
                }
            }
        } else {
