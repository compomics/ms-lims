 ...
 */
public class AlternativeIdentification extends AlternativeIdentificationTableAccessor {
     ...
     * Default constructor.
     */
    public AlternativeIdentification() {
        super();
    }

    public AlternativeIdentification(HashMap aHM) {
        super(aHM);
     ...
     */
    public AlternativeIdentification(ResultSet aRS) throws SQLException {
        this.iAlternativeIdentificationid = aRS.getInt(1);
        this.iL_Identificationid = aRS.getLong(2);
        this.iL_spectrumid = aRS.getLong(3);
        this.iL_datfileid = aRS.getLong(4);
        this.iDatfile_query = aRS.getLong(5);
        this.iAccession = aRS.getString(6);
        this.iStart = aRS.getLong(7);
        this.iEnd = aRS.getLong(8);
        this.iEnzymatic = aRS.getString(9);
        this.iSequence = aRS.getString(10);
        this.iModified_sequence = aRS.getString(11);
        this.iIon_coverage = aRS.getString(12);
        this.iScore = aRS.getLong(13);
        this.iHomology = aRS.getLong(14);
        this.iExp_mass = (Number) aRS.getObject(15);
        this.iCal_mass = (Number) aRS.getObject(16);
        this.iLight_isotope = (Number) aRS.getObject(17);
        this.iHeavy_isotope = (Number) aRS.getObject(18);
        this.iValid = aRS.getInt(19);
        this.iDescription = aRS.getString(20);
        this.iIdentitythreshold = aRS.getLong(21);
        this.iConfidence = (Number) aRS.getObject(22);
        this.iDb = aRS.getString(23);
        this.iTitle = aRS.getString(24);
        this.iPrecursor = (Number) aRS.getObject(25);
        this.iCharge = aRS.getInt(26);
        this.iIsoforms = aRS.getString(27);
        this.iDb_filename = aRS.getString(28);
        this.iMascot_version = aRS.getString(29);
        this.iUsername = aRS.getString(30);
        this.iCreationdate = (java.sql.Timestamp) aRS.getObject(31);
        this.iModificationdate = (java.sql.Timestamp) aRS.getObject(32);
     ...
     */
    public static Vector getAlternativeIdentifications(String aAccession, int aLocation, long aProjectid, Connection aConn) throws SQLException {
        ...
        while (rs.next()) {
            temp.add(new AlternativeIdentification(rs));
        }
        rs.close();
        ps.close();
    ...
    public static String[] getAllUniqueAccessions(Connection aConn) throws SQLException {
        PreparedStatement ps = aConn.prepareStatement("select distinct accession from alternative_identificaton");
        ResultSet rs = ps.executeQuery();
     ...
     */
    public static AlternativeIdentification getAlternativeIdentification(Connection aConn, String aFilename) throws SQLException {
        AlternativeIdentification result = null;
        PreparedStatement ps =
        ...
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
            result = new AlternativeIdentification(rs);
        }
        rs.close();
        ps.close();
        return result;
     ...
     * @throws SQLException when the select failed.
     */
    public static HashMap getAlternativeIdentifications(Connection aConn, Collection aFilenames) throws SQLException {
        ...
        for (Iterator lIterator = aFilenames.iterator(); lIterator.hasNext();) {
            ...
            ResultSet rs = ps.executeQuery();
            AlternativeIdentification id = null;
            while (rs.next()) {
                id = new AlternativeIdentification(rs);
            }
            rs.close();
     ...
     */
    public static AlternativeIdentification[] getAllAlternativeIdentificationsforProject(Connection aConn, long aProjectID, String aWhereAddition) throws SQLException {
        String sql =
        ...
        Vector v = new Vector();
        while (rs.next()) {
            v.add(new AlternativeIdentification(rs));
        }
        rs.close();
        prep.close();
        AlternativeIdentification[] lIDs = new AlternativeIdentification[v.size()];
        v.toArray(lIDs);
     ...
     * @throws SQLException when the retrieving of the identifications went wrong.
     */
    public static AlternativeIdentification[] getAllAlternativeIdentificationsforProjectAndInstrument(Connection aConn, long aProjectID, long aInstrumentID, String aWhereAddition) throws SQLException {
        String sql =
     ...
     * @throws SQLException when the retrieving of the identifications went wrong.
     */
    public static AlternativeIdentification[] getAllAlternativeIdentificationsforWhereclause(Connection aConn, String aWhereAddition) throws SQLException {
        String sql =
    ...
    public String toString() {
        return "" + iAlternativeIdentificationid;
    }
    public HashMap getHashMap() {
        HashMap tempMap = new HashMap();
        tempMap.put("ATERNATIVEIDENTIFICATIONID",this.iAlternativeIdentificationid);
        tempMap.put("SPECTRUMFILENAME",this.iSpectrumfilename);
        tempMap.put("DATFILENAME",iDatfilename);
        tempMap.put("L_IDENTIFICATIONID",this.iL_Identificationid);
