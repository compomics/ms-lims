 ...
 */
public class AlternativeValidationTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
     ...
     */
    protected long iAlternativeValidationid = Long.MIN_VALUE;


    /**
     * This variable represents the contents for the 'l_identificationid' column.
     */
    protected long iL_alternativeidentificationid = Long.MIN_VALUE;
     ...
     */
    public static final String ALTERNATIVEVALIDATIONID = "ALTERNATIVEVALIDATIONID";

    /**
     * This variable represents the key for the 'l_identificationid' column.
     */
    public static final String L_ALTERNATIVEIDENTIFICATIONID = "L_IDENTIFICATIONID";
     ...
     * Default constructor.
     */
    public AlternativeValidationTableAccessor() {
    }
     ...
     */
    public AlternativeValidationTableAccessor(HashMap aParams) {
        if(aParams.containsKey(ALTERNATIVEVALIDATIONID)) {
            this.iAlternativeValidationid = ((Long)aParams.get(ALTERNATIVEVALIDATIONID)).longValue();
        }
        if(aParams.containsKey(L_ALTERNATIVEIDENTIFICATIONID)) {
            this.iL_alternativeidentificationid = ((Long)aParams.get(L_ALTERNATIVEIDENTIFICATIONID)).longValue();
        }
     ...
     * @exception java.sql.SQLException    when the ResultSet could not be read.
     */
    public AlternativeValidationTableAccessor(ResultSet aResultSet) throws SQLException {
        this.iAlternativeValidationid = aResultSet.getLong("alternativevalidationid");
        this.iL_alternativeidentificationid = aResultSet.getLong("l_alternativeidentificationid");
    ...
    public long getValidationid() {
        return this.iAlternativeValidationid;
    }
     ...
     *
     * @param	anAlternativeValidationid	long with the value for the Validationid column.
     */
    public void setAlternativeValidationid(long anAlternativeValidationid) {
        this.iAlternativeValidationid = anAlternativeValidationid;
        this.iUpdated = true;
    }
     ...
     *
     * @param	aL_alternativeidentificationid	long with the value for the L_identificationid column.
     */
    public void setL_alternativeidentificationid(long aL_alternativeidentificationid) {
        this.iL_alternativeidentificationid = aL_alternativeidentificationid;
        this.iUpdated = true;
    }
    ...
    public int delete(Connection aConn) throws SQLException {
        int result = 0;
        try {
            PreparedStatement lStat = aConn.prepareStatement("DELETE FROM alternativevalidation WHERE alternativevalidationid = ?");
            lStat.setLong(1, iAlternativeValidationid);
    ...
    public void retrieve(Connection aConn, HashMap aKeys) throws SQLException {
        ...
        if(!aKeys.containsKey(ALTERNATIVEVALIDATIONID)) {
            throw new IllegalArgumentException("Primary key field 'ALTERNATIVEVALIDATIONID' is missing in HashMap!");
        } else {
            iAlternativeValidationid = ((Long)aKeys.get(ALTERNATIVEVALIDATIONID)).longValue();
        }
        // In getting here, we probably have all we need to continue. So let's...
        PreparedStatement lStat = aConn.prepareStatement("SELECT * FROM alternativevalidation WHERE alternativevalidationid = ?");
        lStat.setLong(1, iAlternativeValidationid);
        ResultSet lRS = lStat.executeQuery();
        int hits = 0;
        while(lRS.next()) {
            hits++;
            iAlternativeValidationid = lRS.getLong("alternativevalidationid");
            iL_alternativeidentificationid = lRS.getLong("l_alternativeidentificationid");
     ...
     */
    public static ArrayList<AlternativeValidationTableAccessor> retrieveAllEntries(Connection aConn) throws SQLException {
        ArrayList<AlternativeValidationTableAccessor>  entities = new ArrayList<AlternativeValidationTableAccessor>();
        Statement stat = aConn.createStatement();
        ResultSet rs = stat.executeQuery(getBasicSelect());
        while(rs.next()) {
            entities.add(new AlternativeValidationTableAccessor(rs));
        }
        rs.close();
        stat.close();
        return entities;
    ...
    public int update(Connection aConn) throws SQLException {
        ...
        int result = 0;
        try {
            PreparedStatement lStat = aConn.prepareStatement("UPDATE alternativevalidation SET alternativevalidationid = ?, l_alternativeidentificationid = ?, l_validationtypeid = ?, auto_comment = ?, manual_comment = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE alternativevalidationid = ?");
            lStat.setLong(1, iAlternativeValidationid);
            lStat.setLong(2, iL_alternativeidentificationid);
            ...
            lStat.setLong(8, iAlternativeValidationid);
            result = lStat.executeUpdate();
            aConn.commit();
            lStat.close();
        } catch (SQLException sqle) {
    ...
    public int persist(Connection aConn) throws SQLException {
        ...
        try {
            ...
            if (iL_alternativeidentificationid == Long.MIN_VALUE) {
                lStat.setNull(2, 4);
            } else {
                lStat.setLong(2, iL_alternativeidentificationid);
            }
            ...
            if (iKeys != null && iKeys.length == 1 && iKeys[0] != null) {
                ...
                iAlternativeValidationid = ((Number) iKeys[0]).longValue();
            }
        } catch (SQLException sqle) {
            aConn.rollback();
            logger.error(sqle);
