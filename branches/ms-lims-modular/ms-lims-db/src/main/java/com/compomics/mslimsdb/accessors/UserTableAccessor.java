...
/**
 * This class is a generated accessor for the User table.
 *
 * @author DBAccessor generator class (Lennart Martens).
 */
public class UserTableAccessor implements Deleteable, Retrievable, Updateable, Persistable {
    // Class specific log4j logger for UserTableAccessor instances.
    private static Logger logger = Logger.getLogger(UserTableAccessor.class);
    ...
    /**
     * This variable represents the contents for the 'userid' column.
     */
    protected long iUserid = Long.MIN_VALUE;
    ...
    /**
     * This variable represents the key for the 'userid' column.
     */
    ...
    public UserTableAccessor(HashMap aParams) {
        if (aParams.containsKey(USERID)) {
            this.iUserid = ((Long) aParams.get(USERID)).longValue();
        }
    ...
    /**
     * This method returns the value for the 'Userid' column
     *
     * @return long    with the value for the Userid column.
     */
    ...
    public int delete(Connection aConn) throws SQLException {
        PreparedStatement lStat = aConn.prepareStatement("DELETE FROM user WHERE userid = ?");
        lStat.setLong(1, iUserid);
    ...
    public void retrieve(Connection aConn, HashMap aKeys) throws SQLException {
        ...
        if (!aKeys.containsKey(USERID)) {
            throw new IllegalArgumentException("Primary key field 'USERID' is missing in HashMap!");
        } else {
            iUserid = ((Long) aKeys.get(USERID)).longValue()
                    ;
        }
        // In getting here, we probably have all we need to continue. So let's...
        PreparedStatement lStat = aConn.prepareStatement("SELECT * FROM user WHERE userid = ?");
        lStat.setLong(1, iUserid);
        ResultSet lRS = lStat.executeQuery();
        int hits = 0;
        while (lRS.next()) {
            hits++;
            iUserid = lRS.getLong("userid");
        ...
        lStat.close();
        if (hits > 1) {
            throw new SQLException("More than one hit found for the specified primary keys in the 'user' table! Object is initialized to last row returned.");
        } else if (hits == 0) {
            throw new SQLException("No hits found for the specified primary keys in the 'user' table! Object is not initialized correctly!");
        }
    }
    ...
    public int update(Connection aConn) throws SQLException {
        ...
        PreparedStatement lStat = aConn.prepareStatement("UPDATE user SET userid = ?, name = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE userid = ?");
        lStat.setLong(1, iUserid);
        lStat.setObject(2, iName);
        lStat.setObject(3, iUsername);
        lStat.setObject(4, iCreationdate);
        lStat.setLong(5, iUserid);
        int result = lStat.executeUpdate();
        lStat.close();
        this.iUpdated = false;
        return result;
    ...
    public int persist(Connection aConn) throws SQLException {
        PreparedStatement lStat = aConn.prepareStatement("INSERT INTO user (userid, name, username, creationdate, modificationdate) values(?, ?, CURRENT_USER, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)",Statement.RETURN_GENERATED_KEYS);
        if (iUserid == Long.MIN_VALUE) {
            lStat.setNull(1, 4);
        } else {
            lStat.setLong(1, iUserid);
        }
