...
public class AlternativeFragmentIonTableAccessor  implements Deleteable, Retrievable, Updateable, Persistable {
     ...
     */
    protected long iAlternativeFragmentionid = Long.MIN_VALUE;
     ...
     */
    public AlternativeFragmentIonTableAccessor(HashMap aParams) {
        if (aParams.containsKey(ALTERNATIVEFRAGMENTIONID)) {
            this.iAlternativeFragmentionid = ((Long) aParams.get(ALTERNATIVEFRAGMENTIONID)).longValue();
        }
    ...
    public void setFragmentionid(long aFragmentionid) {
        this.iAlternativeFragmentionid = aFragmentionid;
        this.iUpdated = true;
    }
    ...
    public void setL_identificationid(long aL_identificationid) {
        this.iL_alternativeidentificationid = aL_identificationid;
        this.iUpdated = true;
    }
    ...
    public int delete(Connection aConn) throws SQLException {
        int result = 0;
        try {
            PreparedStatement lStat = aConn.prepareStatement("DELETE FROM alternativefragmention WHERE alternativefragmentionid = ?");
            lStat.setLong(1, iAlternativeFragmentionid);
    ...
    public void retrieve(Connection aConn, HashMap aKeys) throws SQLException {
        ...
        if (!aKeys.containsKey(ALTERNATIVEFRAGMENTIONID)) {
            throw new IllegalArgumentException("Primary key field 'ALTERNATIVEFRAGMENTIONID' is missing in HashMap!");
        } else {
            iAlternativeFragmentionid = ((Long) aKeys.get(ALTERNATIVEFRAGMENTIONID)).longValue()
            ;
        }
        // In getting here, we probably have all we need to continue. So let's...
        PreparedStatement lStat = aConn.prepareStatement("SELECT * FROM alternativefragmention WHERE alternativefragmentionid = ?");
        lStat.setLong(1, iAlternativeFragmentionid);
        ResultSet lRS = lStat.executeQuery();
        int hits = 0;
        while (lRS.next()) {
            hits++;
            iAlternativeFragmentionid = lRS.getLong("alternativefragmentionid");
    ...
    public int update(Connection aConn) throws SQLException {
        ...
        int result = 0;
        try {
            PreparedStatement lStat = aConn.prepareStatement("UPDATE alternativefragmention SET alternativefragmentionid = ?, l_alternativeidentificationid = ?, iontype = ?, ionname = ?, l_ionscoringid = ?, mz = ?, intensity = ?, fragmentionnumber = ?, massdelta = ?, masserrormargin = ?, username = ?, creationdate = ?, modificationdate = CURRENT_TIMESTAMP WHERE fragmentionid = ?");
            lStat.setLong(1, iAlternativeFragmentionid);
            ...
            lStat.setLong(13, iAlternativeFragmentionid);
            result = lStat.executeUpdate();
            aConn.commit();
            lStat.close();
        } catch (SQLException sqle) {
