 ...
 */
public class LoadMGFWorker extends SwingWorker {
    // Class specific log4j logger for LoadMGFWorker instances.
    private static Logger logger = Logger.getLogger(LoadMGFWorker.class);
     ...
     */
    public LoadMGFWorker(File[] aList, Vector aStored, Vector aNames, Flamable aParent, DefaultProgressBar aProgress) {
        list = aList;
        iStoredLCs = aStored;
        names = aNames;
        outer = aParent;
    ...
    public Object construct() {
        for (int i = 0; i < list.length; i++) {
            ...
            try {
                Object temp = MergeFileReaderFactory.getReaderForMergeFile(file);
