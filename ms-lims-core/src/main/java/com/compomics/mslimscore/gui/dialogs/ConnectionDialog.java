 ...
 */
/**
 * This class implements a dialog to gather all information concerning a DB
 * connection.
 * 
...
// Class specific log4j logger for ConnectionDialog instances.
    ...
    /**
     * *
     * ArrayList that holds all the preconfigured connections. Note that when
     * this list holds only 0 or 1 elements, it changes the behaviour of the
     * GUI.
     */
    ...
    /**
     * This constructor takes as arguments the parent JFrame and a title for the
     * dialog. It also constructs the GUI.
     *     
* @param aParent JFrame that is the parent of this JDialog.
     * @param aTarget Connectable to which the connection will be passed once it
     * is successfully established.
     * @param aTitle String with the title for this dialog
     * @param aPropsFile String with the filename for a propertiesfile with some
     * connection parameters already filled out, notably 'url' and 'driver'. Can
     * be 'null' for no defaults. If something goes wrong, no defaults are
     * filled out.
     */
...
* @param aParent JFrame that is the parent of this JDialog.
     ...
     * is successfully established.
     * @param aTitle String with the title for this dialog
     * @param aConnectionProperties Properties instance with three variables
     * (name, driver, url)
     */
    ...
    /**
     * This method actually shows the ConnectionDialog. It takes care of the GUI
     * related stuff.
     */
...
// Here we have to set the defaults.
            if (iConnections.size() > 0) {
                ConnectionDialog.InnerConfigParams params = (ConnectionDialog.InnerConfigParams) iConnections.get(0);
            ...
            cmbConfigurations.addItemListener(new ItemListener() {
                public void itemStateChanged(ItemEvent e) {
                    ...
                    if (temp != null) {
                        ConnectionDialog.InnerConfigParams params = (ConnectionDialog.InnerConfigParams) temp;
                        initConfiguration(params);
                    }
                }
            });
            initConfiguration((ConnectionDialog.InnerConfigParams) iConnections.get(0));
    ...
    /**
     * This method initializes the predefined connection parameters 'driver' and
     * 'url', if they are found in the specified InnerConfigParams.
     *     
* @param aParams InnerConfigParams with the predefined configuration
     * parameters to initialize.
     */
    private void initConfiguration(ConnectionDialog.InnerConfigParams aParams) {
...
// Now continue into ms_lims.
    ...
    /**
     * This method attempts to load connection parameters from a properties file
     * in the classpath. If this file is not found, nothing happens. If it is
     * found, the parameters found will be filled out.
     */
    private void tryToLoadParams() {
        if (iPropsFile != null) {
            try {
                Properties p = new Properties();
                InputStream is = new FileInputStream(new File(PeptizerGUI.class.getProtectionDomain().getCodeSource().getLocation().getPath() + (iPropsFile)));
                if (is == null) {
...
// Do nothing.
    ...
    /**
     * This method reads the predefined connection properties from a Properties
     * instance.
     *     
* @param aConnectionProperties Properties instance with the information for
     * the connection(s)
     */
...
// which will be stored under the 'DEFAULT' key in the hashmap.
            ...
            iConnections.add(new ConnectionDialog.InnerConfigParams(dbUser, "DEFAULT", dbDriver, dbUrl));
        } else {
            ...
            for (int i = 0; i < configurations.length; i++) {
                ...
                iConnections.add(new ConnectionDialog.InnerConfigParams(dbUser, lConfiguration, dbDriver, dbUrl));
            }
        }
    }
