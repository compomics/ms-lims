...
/**
 * This class is used to store the headers of isoforms from a Mascot results file using MascotID.
 *
 * @author Lennart Martens
 */
public class MascotIsoforms {
    // Class specific log4j logger for MascotIsoforms instances.
    private static Logger logger = Logger.getLogger(MascotIsoforms.class);
    ...
    /**
     * This method stores an isoform in the collection if it is new (based on accession), or will update start and end
     * if already present.
     *
     * @param aAccession   String with the accession number (may contain location information: ' (x-y)' at the end).
    ...
    /**
     * This method will return a MascotHeader with the accession that was found first from the HashMap (if available)
     * and the first (natural ordering courtesy of MascotHeader class) otherwise. If the specified accession numbers are
     * present, but they score less than another accession number present, the highest scoring one is returned.
     *
    ...
    /**
     * This method returns the MascotHeader for the specified accession number, or 'null' if no match found.
     *
     * @param aAccession String with the accession number to retrieve.
     * @return MascotHeader    with the mascot header for the specified accession number, or 'null' if no match.
     */
